...
function getTargetAlias(targets, serverAlias) {
...
function resolveSubdirectoryMapping(sub, relativePath, preferredServerAlias) {
  ...
  let defaultTargetHint = null;
  let serverAlias = preferredServerAlias || sub.defaultServerAlias || sub.serverAlias || (aliases.length === 1 ? aliases[0] : null);

  // 多目标未指定 --server 时，默认使用第一条并给出提示
  if (!serverAlias && aliases.length > 1) {
    serverAlias = aliases[0];
    defaultTargetHint = aliases;
  }

  const targetAlias = getTargetAlias(targets, serverAlias);
  if (!targetAlias) {
    return {
      relativePath,
  ...
  const result = resolveTargetValue(targets[targetAlias], serverAlias, relativePath);
  if (defaultTargetHint) {
    result.defaultTargetHint = defaultTargetHint;
  }
  return result;
}
 ...
 * @returns {{ remoteBase?: string, relativePath: string, serverAlias?: string, ambiguousTargets?: string[], defaultTargetHint?: string[], targetNotFound?: string } | null}
 */
function resolveRemotePath(localPath, mapping, repoRoot, preferredServerAlias) {
  ...
  if (mapping.subdirectoryMappings && Object.keys(mapping.subdirectoryMappings).length > 0) {
    ...
    const repoDirName = path.basename(repoRoot).replace(/\\/g, '/');
    const candidatePaths = relativePath
      ? [relativePath, `${repoDirName}/${relativePath}`]
      : ['', repoDirName];

    for (const candidatePath of candidatePaths) {
      const matched = normalizedMappings.find(item => (
        candidatePath === item.normalized || candidatePath.startsWith(`${item.normalized}/`)
      ));

      if (matched) {
        const sub = mapping.subdirectoryMappings[matched.raw];
        const subRelativePath = candidatePath === matched.normalized
          ? ''
          : candidatePath.slice(matched.normalized.length + 1);
